...
export function commandToChoice(command){
  ...
  if ("Mode" in command && ((command["Mode"].includes("DTS")) || (command["Mode"].includes("UNII")))){// is wifi
    ...
    if (command["Subband-Index"] !== undefined){
      choiceDict["Subband-Index"] = command["Subband-Index"];
    }
